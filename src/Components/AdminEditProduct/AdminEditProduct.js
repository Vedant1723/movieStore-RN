...
const AdminEditProduct = () => {
  const {
    authContext: {updateMovie},
    state: {movie},
  } = useContext(UserContext);

  const [mName, setMName] = useState(movie.mName || '');
  const [mImage, setMImage] = useState('');
  const [mType, setMType] = useState(movie.mType || '');
  const [mDescription, setMDescription] = useState(movie.mDescription || '');
  const [mTrailer, setMTrailer] = useState(movie.mTrailer || '');
  ...
  const submitData = () => {
    ...
    updateMovie(formData, movie._id);
  };
  return (
